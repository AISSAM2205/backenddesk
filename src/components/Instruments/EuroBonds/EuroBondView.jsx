...
const Row = ({ r, idx }) => {
  ...
  const td = { padding: '5px 8px', borderBottom: '1px solid var(--b0)', whiteSpace: 'nowrap', transition: 'background 0.1s' };
  const nb = { ...td, textAlign: 'right', fontFamily: 'var(--f-mono)', fontSize: '0.67rem', fontVariantNumeric: 'tabular-nums' };
  ...
  return (
    <tr className={flash} style={{ background: rowBg }} onMouseEnter={e => e.currentTarget.style.background = 'var(--tr-hover-bg)'} onMouseLeave={e => e.currentTarget.style.background = rowBg}>
      ...
      <td style={{ ...nb, color: '#FCD34D' }}>
        {fN(r.gSpreadBid, 1)}
        {r.gSpreadBid != null && r.targetSpread != null && parseFloat(r.targetSpread) > 0 && (() => {
          const gap = parseFloat(r.gSpreadBid) - parseFloat(r.targetSpread);
          return <span style={{ fontFamily: 'var(--f-mono)', fontSize: '0.54rem', marginLeft: 4, color: gap > 0 ? 'var(--profit)' : 'var(--loss)', opacity: 0.9 }}>{gap > 0 ? `+${gap.toFixed(0)}` : gap.toFixed(0)}</span>;
        })()}
      </td>
      ...
      <td style={{ ...nb }}>
        {r.targetSpread != null ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, justifyContent: 'flex-end' }}>
            ...
            {r.gSpreadBid != null && (
              <div style={{ position: 'relative', width: 28, height: 4, background: 'var(--elev)', borderRadius: 2, overflow: 'visible', flexShrink: 0 }}>
                <div style={{ position: 'absolute', left: 0, top: 0, height: '100%', width: `${Math.min((parseFloat(r.gSpreadBid) / Math.max(parseFloat(r.gSpreadBid), parseFloat(r.targetSpread), 1)) * 100, 100)}%`, background: r.decision === 'BUY' ? 'var(--profit)' : 'var(--cyan)', borderRadius: 2, opacity: 0.75 }} />
              </div>
            )}
          </div>
        ) : '—'}
      </td>
      <td style={{ ...nb, color: '#FCD34D' }}>{r.yieldToMaturity != null ? fN(r.yieldToMaturity, 3) + '%' : '—'}</td>
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', background: 'var(--void)' }}>
      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 20, background: 'var(--void)', borderBottom: '1px solid var(--b1)' }}>
        ...
        <div style={{ padding: '8px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
            <span style={{ display: 'inline-block', width: 2, height: 13, background: 'var(--eb)', borderRadius: 1, flexShrink: 0 }} />
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <h2 className="view-title">Market Watch — EuroBonds</h2>
                <span className="tag tag-blue">{sorted.length} / {dashboardRows.length} ISIN</span>
              </div>
              <p className="view-sub">{COLS.length} colonnes Bloomberg · {selectedDate}</p>
            </div>
          </div>
        ...
        {/* Filter row */}
        <div style={{ padding: '6px 16px 8px', display: 'flex', alignItems: 'center', gap: 8, borderTop: '1px solid var(--b0)', flexWrap: 'wrap' }}>
          <div style={{ position: 'relative', flexShrink: 0 }}>
            ...
            <input
              ...
              style={{
                background: 'var(--surf)', border: '1px solid var(--b1)', borderRadius: 4,
                padding: '4px 9px 4px 26px', color: 'var(--tx1)',
                fontFamily: 'var(--f-body)', fontSize: '0.70rem', outline: 'none', width: 200,
              }}
            />
          </div>
          ...
          {currencies.length > 1 && (
            <select value={filterCcy} onChange={e => setFilterCcy(e.target.value)} className="select" style={{ height: 24, borderRadius: 3 }}>
      ...
      {sorted.length > 0 && (() => {
        ...
        const totalCarry = sorted.reduce((s, r) => s + parseFloat(r.netDailyMad || 0), 0);
        const ytmVals = sorted.map(r => parseFloat(r.yieldToMaturity)).filter(n => !isNaN(n) && n > 0);
        const avgYtm = ytmVals.length > 0 ? ytmVals.reduce((s, v) => s + v, 0) / ytmVals.length : null;
        const bestOpp = sorted
          .filter(r => r.gSpreadBid != null && r.targetSpread != null && parseFloat(r.targetSpread) > 0)
          .map(r => ({ ...r, gap: parseFloat(r.gSpreadBid) - parseFloat(r.targetSpread) }))
          .sort((a, b) => b.gap - a.gap)[0];

        const Sep2 = () => <span style={{ width: 1, height: 22, background: 'var(--b1)', flexShrink: 0 }} />;

        return (
          <div style={{
            padding: '6px 16px', background: 'var(--surf)', borderBottom: '1px solid var(--b1)',
            display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap', overflowX: 'auto',
          }}>
            {/* BUY signals */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, flexShrink: 0 }}>
              <span className="lbl">Opportunités</span>
              {buySignals.length > 0
                ? <span className="badge badge-active">▲ {buySignals.length} BUY</span>
            ...
            {bestOpp && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                ...
                <span style={{ fontFamily: 'var(--f-mono)', fontSize: '0.67rem', fontWeight: 700, color: bestOpp.gap > 0 ? 'var(--profit)' : 'var(--loss)' }}>
                  {bestOpp.gap > 0 ? '+' : ''}{bestOpp.gap.toFixed(1)} bp
                </span>
                <span style={{ fontFamily: 'var(--f-body)', fontSize: '0.65rem', color: 'var(--tx3)', maxWidth: 130, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={bestOpp.description}>
                  {(bestOpp.description || bestOpp.isin).split(' ').slice(0, 3).join(' ')}
                </span>
              </div>
            )}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
              ...
              <span style={{ fontFamily: 'var(--f-mono)', fontSize: '0.68rem', fontWeight: 600, color: totalCarry >= 0 ? 'var(--profit)' : 'var(--loss)' }}>
            ...
            {avgYtm != null && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                ...
                <span style={{ fontFamily: 'var(--f-mono)', fontSize: '0.68rem', color: '#FCD34D' }}>
                  {avgYtm.toFixed(3)}%
                </span>
              </div>
            )}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
              ...
              <span style={{ fontFamily: 'var(--f-mono)', fontSize: '0.68rem', color: '#60A5FA' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
              ...
              <span style={{ fontFamily: 'var(--f-mono)', fontSize: '0.68rem', color: 'var(--tx1)', fontWeight: 500 }}>
      ...
      {(() => {
        const eligible = sorted.filter(r =>
          r.gSpreadMid != null && r.targetSpread != null && parseFloat(r.targetSpread) > 0
        );
        ...
        const maxAbs = scored.reduce((m, r) => Math.max(m, Math.abs(r.score)), 1);
        const cellBg = (v) => {
          ...
          <div style={{ padding: '0 16px 16px' }}>
            <div className="card" style={{ padding: '14px 16px', overflow: 'hidden' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                <p className="sect-ttl" style={{ margin: 0 }}>Matrice G-Spread — Relative Value</p>
                <span style={{ fontFamily: 'var(--f-body)', fontSize: '0.60rem', color: 'var(--tx3)' }}>
              ...
              <div style={{ overflowX: 'auto' }}>
                <table style={{ borderCollapse: 'collapse', width: '100%', minWidth: scored.length * 80 + 180 }}>
                  <thead>
                    <tr>
                      <th style={{ textAlign: 'left', padding: '4px 8px', fontFamily: 'var(--f-disp)', fontSize: '0.55rem', fontWeight: 700, letterSpacing: '0.08em', color: 'var(--tx3)', textTransform: 'uppercase', borderBottom: '1px solid var(--b1)', minWidth: 160 }}>
                        ISIN
                      </th>
                      <th style={{ textAlign: 'right', padding: '4px 8px', fontFamily: 'var(--f-disp)', fontSize: '0.55rem', fontWeight: 700, letterSpacing: '0.08em', color: 'var(--tx3)', textTransform: 'uppercase', borderBottom: '1px solid var(--b1)' }}>
                        G-Spd Mid
                      </th>
                      <th style={{ textAlign: 'right', padding: '4px 8px', fontFamily: 'var(--f-disp)', fontSize: '0.55rem', fontWeight: 700, letterSpacing: '0.08em', color: 'var(--tx3)', textTransform: 'uppercase', borderBottom: '1px solid var(--b1)' }}>
                        Target
                      </th>
                      <th style={{ textAlign: 'right', padding: '4px 8px', fontFamily: 'var(--f-disp)', fontSize: '0.55rem', fontWeight: 700, letterSpacing: '0.08em', color: 'var(--tx3)', textTransform: 'uppercase', borderBottom: '1px solid var(--b1)', minWidth: 200 }}>
                        Cheapness Score (bp)
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {scored.map((r, i) => (
                      <tr key={r.isin} style={{ background: i % 2 === 0 ? 'var(--tr-even-bg)' : 'transparent' }}>
                        <td style={{ padding: '5px 8px', fontFamily: 'var(--f-mono)', fontSize: '0.66rem', color: 'var(--cyan)', fontWeight: 500 }}>
                          <span>{r.isin}</span>
                          <span style={{ fontFamily: 'var(--f-body)', fontSize: '0.58rem', color: 'var(--tx3)', marginLeft: 6 }}>{r.desc}</span>
                        </td>
                        <td style={{ padding: '5px 8px', textAlign: 'right', fontFamily: 'var(--f-mono)', fontSize: '0.66rem', color: '#FCD34D' }}>{r.mid.toFixed(1)}</td>
                        <td style={{ padding: '5px 8px', textAlign: 'right', fontFamily: 'var(--f-mono)', fontSize: '0.66rem', color: 'var(--tx2)' }}>{r.target.toFixed(1)}</td>
                        <td style={{ padding: '5px 8px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            <div style={{ flex: 1, height: 8, background: 'var(--b1)', borderRadius: 4, overflow: 'hidden', position: 'relative' }}>
                              <div style={{
                                position: 'absolute', top: 0, height: '100%',
                                width: `${Math.min(Math.abs(r.score) / maxAbs * 100, 100)}%`,
                                background: cellCol(r.score), borderRadius: 4, opacity: 0.7,
                                ...(r.score >= 0 ? { left: 0 } : { right: 0 }),
                              }} />
                            </div>
                            <span style={{
                              fontFamily: 'var(--f-mono)', fontWeight: 700, fontSize: '0.70rem',
                              color: cellCol(r.score), minWidth: 52, textAlign: 'right', flexShrink: 0,
                            }}>
                              {r.score >= 0 ? '+' : ''}{r.score.toFixed(1)} bp
                            </span>
                            {r.score > 0 && (
                              <span style={{ fontFamily: 'var(--f-disp)', fontSize: '0.52rem', fontWeight: 700, color: 'var(--profit)', background: 'rgba(0,232,153,0.12)', border: '1px solid rgba(0,232,153,0.25)', borderRadius: 3, padding: '1px 5px', flexShrink: 0 }}>
                                CHEAP
                              </span>
                            )}
                            {r.score < -5 && (
                              <span style={{ fontFamily: 'var(--f-disp)', fontSize: '0.52rem', fontWeight: 700, color: 'var(--loss)', background: 'rgba(255,43,96,0.12)', border: '1px solid rgba(255,43,96,0.25)', borderRadius: 3, padding: '1px 5px', flexShrink: 0 }}>
                                RICH
                              </span>
                            )}
                          </div>
      ...
      {sorted.length > 0 && (() => {
        const bonds = sorted.filter(r =>
          r.gSpreadMid != null || r.iSpreadBid != null || r.yieldToMaturity != null
        );
        ...
        const rows = bonds.map(r => {
          ...
          const daysToMat   = matDate ? Math.max(0, Math.round((matDate - today) / 86400000)) : null;
          ...
          return { ...r, gMid, iBid, gBid, asw, ytm, dur, cpn, nominal, rollYield, daysToMat, yearsToMat, spreadGap };
        });

        const TH2 = { padding: '7px 10px', fontFamily: 'var(--f-disp)', fontWeight: 700, fontSize: '0.52rem', letterSpacing: '0.09em', textTransform: 'uppercase', color: 'var(--tx3)', borderBottom: '1px solid var(--b1)', whiteSpace: 'nowrap' };
        const TD2 = { padding: '7px 10px', fontFamily: 'var(--f-mono)', fontSize: '0.66rem', borderBottom: '1px solid var(--b0)', whiteSpace: 'nowrap', fontVariantNumeric: 'tabular-nums', textAlign: 'right' };
            ...
            <div className="card" style={{ overflow: 'hidden' }}>
              <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--b1)', display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontFamily: 'var(--f-disp)', fontWeight: 700, fontSize: '0.65rem', letterSpacing: '0.10em', textTransform: 'uppercase', color: 'var(--tx1)' }}>
                  Analyse Spread — I-Spread / G-Spread / Roll · {bonds.length} ISINs
                </span>
                <span style={{ marginLeft: 'auto', fontFamily: 'var(--f-disp)', fontSize: '0.52rem', fontWeight: 700, color: 'rgba(232,154,32,0.70)', letterSpacing: '0.06em', padding: '2px 6px', borderRadius: 3, background: 'rgba(232,154,32,0.08)', border: '1px solid rgba(232,154,32,0.20)' }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ background: 'var(--thead-bg)' }}>
                      ...
                      <th style={{ ...TH2, textAlign: 'left' }}>Description</th>
                      <th style={TH2}>CCY</th>
                      <th style={TH2}>G-Spread<br/><span style={{ fontWeight: 400, opacity: 0.6 }}>Mid (bp)</span></th>
                      <th style={TH2}>I-Spread<br/><span style={{ fontWeight: 400, opacity: 0.6 }}>Bid (bp)</span></th>
                      <th style={TH2}>ASW<br/><span style={{ fontWeight: 400, opacity: 0.6 }}>(bp)</span></th>
                      <th style={TH2}>G−I<br/><span style={{ fontWeight: 400, opacity: 0.6 }}>basis</span></th>
                      <th style={TH2}>YTM %</th>
                      <th style={TH2}>Dur.<br/><span style={{ fontWeight: 400, opacity: 0.6 }}>mod (ans)</span></th>
                      <th style={TH2}>J−Éch.</th>
                      <th style={TH2}>Roll-down<br/><span style={{ fontWeight: 400, opacity: 0.6 }}>estim. (bp)</span></th>
                      <th style={TH2}>% Encours<br/><span style={{ fontWeight: 400, opacity: 0.6 }}>Outstanding</span></th>
                  ...
                  <tbody>
                    {rows.map((r, i) => {
                      ...
                      const giBasis = r.spreadGap !== null ? parseFloat(r.spreadGap) : null;
                      const giBasisCol = giBasis === null ? 'var(--tx3)' : Math.abs(giBasis) > 20 ? 'var(--warn)' : 'var(--tx2)';
                      const cheapScore = r.targetSpread ? (r.gBid - parseFloat(r.targetSpread)).toFixed(1) : null;
                      const isCheap = cheapScore !== null && parseFloat(cheapScore) > 5;
                      const isRich  = cheapScore !== null && parseFloat(cheapScore) < -5;
                      return (
                        <tr key={r.isin + i} style={{ background: rowBg }}
                          onMouseEnter={e => e.currentTarget.style.background = 'var(--tr-hover-bg)'}
                          onMouseLeave={e => e.currentTarget.style.background = rowBg}>
                          <td style={{ ...TD2, textAlign: 'left', color: 'var(--cyan)', fontWeight: 500 }}>{r.isin}</td>
                          <td style={{ ...TD2, textAlign: 'left', color: 'var(--tx1)', maxWidth: 130, overflow: 'hidden', textOverflow: 'ellipsis' }} title={r.description}>
                            {(r.description || '').split(' ').slice(0, 4).join(' ') || '—'}
                          </td>
                          <td style={{ ...TD2, textAlign: 'center', color: 'var(--tx3)', fontFamily: 'var(--f-disp)', fontWeight: 700, fontSize: '0.60rem' }}>{r.currency || '—'}</td>
                          <td style={{ ...TD2, color: '#FCD34D', fontWeight: 600 }}>{r.gMid > 0 ? r.gMid.toFixed(1) : '—'}</td>
                          <td style={{ ...TD2, color: r.iBid > 0 ? '#C084FC' : 'var(--tx3)' }}>{r.iBid > 0 ? r.iBid.toFixed(1) : '—'}</td>
                          <td style={{ ...TD2, color: r.asw !== 0 ? '#60A5FA' : 'var(--tx3)' }}>{r.asw !== 0 ? r.asw.toFixed(1) : '—'}</td>
                          <td style={{ ...TD2, color: giBasisCol }}>
                            {giBasis !== null ? (giBasis >= 0 ? `+${giBasis.toFixed(1)}` : giBasis.toFixed(1)) : '—'}
                          </td>
                          <td style={{ ...TD2, color: '#FCD34D' }}>{r.ytm > 0 ? `${r.ytm.toFixed(3)}%` : '—'}</td>
                          <td style={{ ...TD2, color: '#C084FC' }}>{r.dur > 0 ? r.dur.toFixed(2) : '—'}</td>
                          <td style={{ ...TD2, color: 'var(--tx2)' }}>
                            {r.daysToMat !== null ? (
                              <span>
                                {r.daysToMat}
                                <span style={{ color: 'var(--tx3)', fontSize: '0.56rem', marginLeft: 3 }}>j</span>
                              </span>
                            ) : '—'}
                          </td>
                          <td style={{ ...TD2, color: 'var(--profit)' }}>{r.rollYield > 0 ? `+${r.rollYield.toFixed(1)}` : '—'}</td>
                          <td style={{ ...TD2, color: 'var(--tx3)' }}>
                            <span style={{ fontFamily: 'var(--f-disp)', fontSize: '0.52rem', color: 'rgba(232,154,32,0.55)', letterSpacing: '0.05em' }}>Bloomberg</span>
                          </td>
                          <td style={{ ...TD2, textAlign: 'center' }}>
                            {isCheap && <span style={{ fontFamily: 'var(--f-disp)', fontSize: '0.52rem', fontWeight: 700, color: 'var(--profit)', background: 'rgba(0,232,153,0.10)', border: '1px solid rgba(0,232,153,0.25)', borderRadius: 3, padding: '2px 5px' }}>CHEAP</span>}
                            {isRich  && <span style={{ fontFamily: 'var(--f-disp)', fontSize: '0.52rem', fontWeight: 700, color: 'var(--loss)',   background: 'rgba(255,43,96,0.10)', border: '1px solid rgba(255,43,96,0.25)',   borderRadius: 3, padding: '2px 5px' }}>RICH</span>}
                            {!isCheap && !isRich && <span style={{ color: 'var(--tx3)', fontSize: '0.60rem' }}>—</span>}
                          </td>
                        </tr>
                      );
                    })}
              ...
              <div style={{ padding: '7px 14px', borderTop: '1px solid var(--b0)' }}>
                <span style={{ fontFamily: 'var(--f-body)', fontSize: '0.57rem', color: 'var(--tx3)' }}>
                  G-Spread = spread vs UST courbe · I-Spread = spread vs swap · ASW = asset swap spread · G−I basis : delta entre les deux benchmarks · Roll-down = gain estimé en duration / maturité · % Outstanding en attente flux Bloomberg BDP
                </span>
              </div>
            </div>
          </div>
