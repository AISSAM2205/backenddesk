...
function computeGlobal(rows) {
  ...
  const totalPnlAccountingMad = rows.reduce((s, r) => s + n(r.pnlAccountingMad),  0);
  ...
  const durRows  = rows.filter(r => r.modifiedDuration != null && n(r.netNominal) > 0);
  const nomTotal = durRows.reduce((s, r) => s + n(r.netNominal), 0);
  const portfolioDuration = nomTotal > 0
    ? durRows.reduce((s, r) => s + n(r.modifiedDuration) * n(r.netNominal), 0) / nomTotal
    : null;
  ...
  rows.forEach(r => {
    ...
    const key = sub.includes('cln')                          ? 'CLN'
              : sub.includes('egp') || sub.includes('bill') ? 'EGP_BILL'
              : sub.includes('future')                       ? 'FUTURES'
              : 'EUROBOND';
...
function reducer(state, action) {
  switch (action.type) {
    case 'SET_ACTIVE_INSTRUMENT':
      return { ...state, activeInstrument: action.payload, activeSection: 'overview' };
    case 'SET_ACTIVE_SECTION':    return { ...state, activeSection: action.payload };
    case 'SET_DATE':              return { ...state, selectedDate: action.payload };
    case 'SET_DASHBOARD_ROWS':    return { ...state, dashboardRows: action.payload };
    case 'SET_GLOBAL_DASHBOARD':  return { ...state, globalDashboard: action.payload };
    case 'SET_RATES':             return { ...state, rates: action.payload };
    case 'SET_RISK':              return { ...state, riskData: action.payload };
    case 'SET_DURATION':          return { ...state, portfolioDuration: action.payload };
    case 'SET_CLN':               return { ...state, clnList: action.payload };
    case 'SET_EGP':               return { ...state, egpList: action.payload };
    case 'SET_TRADES':            return { ...state, trades: action.payload };
    case 'SET_PNL_DAILY':         return { ...state, pnlDailyHistory: action.payload };
...
export const TradingProvider = ({ children }) => {
  ...
  const loadAll = useCallback(async (date) => {
    ...
    const startDate = (() => {
      const d = new Date(endDate); d.setDate(d.getDate() - 90);
      return d.toISOString().split('T')[0];
    })();

    try {
      const [dashRes, globalRes, ratesRes, riskRes, durationRes, clnRes, egpRes, pnlDailyRes] =
      ...
      if (globalRes.status === 'fulfilled' && globalRes.value.data) {
        dispatch({ type: 'SET_GLOBAL_DASHBOARD', payload: globalRes.value.data });
      } else if (rows.length > 0) {
        dispatch({ type: 'SET_GLOBAL_DASHBOARD', payload: computeGlobal(rows) });
      }
      ...
      if (durationRes.status === 'fulfilled' && durationRes.value.data != null) {
        ...
        if (rows.length > 0) {
          const g = globalRes.status === 'fulfilled' && globalRes.value.data
            ? globalRes.value.data
            : computeGlobal(rows);
          dispatch({
            type: 'SET_GLOBAL_DASHBOARD',
            payload: { ...g, portfolioDuration: parseFloat(durationRes.value.data) },
          });
        }
      }
      ...
      if (pnlDailyRes.status === 'fulfilled') {
        dispatch({ type: 'SET_PNL_DAILY', payload: pnlDailyRes.value.data || [] });
      }

      dispatch({ type: 'SET_LAST_UPDATE', payload: new Date() });

    } catch (err) {
      dispatch({ type: 'SET_ERROR', payload: 'Erreur chargement : ' + err.message });
    } finally {
  ...
  return (
    <TradingContext.Provider value={{
      ...state,
      setActiveInstrument: id  => dispatch({ type: 'SET_ACTIVE_INSTRUMENT', payload: id }),
      setActiveSection:    id  => dispatch({ type: 'SET_ACTIVE_SECTION',    payload: id }),
