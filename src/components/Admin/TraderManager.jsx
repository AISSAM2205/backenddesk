...
const PERMISSION_OPTS = [
  {
    ...
    label: "Admin",
    color: "badge-sell",
    desc: "Accès panneau admin",
  },
  {
    key: "EUROBOND_ACCESS",
    label: "EuroBonds",
    color: "badge-eb",
    desc: "Dashboard EuroBonds",
  },
  {
    key: "CLN_ACCESS",
    label: "CLN",
    color: "badge-cln",
    desc: "Credit Linked Notes",
  },
  {
    key: "EGP_ACCESS",
    label: "EGP Bills",
    color: "badge-egp",
    desc: "Bons du Trésor EGP",
  },
  {
    key: "BLOTTER_ACCESS",
    label: "Blotter",
    color: "badge-active",
    desc: "Trade Blotter & Historique",
  },
];
...
const TraderManager = () => {
  ...
  const openEdit = (t) => {
    setEditing(t.id);
    setAdding(false);
    setForm({ ...t, permissions: t.permissions || [] });
  };
  const cancel = () => {
    setAdding(false);
    setEditing(null);
  ...
  const del = (t) => {
    if (window.confirm(`Supprimer ${t.firstName} ${t.lastName} ?`))
      deleteTrader(t.id);
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      >
        <div>
          ...
          >
            {traders.length} membre{traders.length !== 1 ? "s" : ""} —
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {PERMISSION_OPTS.map((opt) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`badge ${active ? opt.color : "badge-closed"}`}
                      ...
                      >
                        {(t.permissions || []).slice(0, 3).map((p) => {
                          ...
                          return (
                            <span
                              key={p}
                              className={`badge ${opt?.color || "badge-closed"}`}
                              style={{ fontSize: "0.56rem" }}
                            >
                              {opt?.label || p.replace("_ACCESS", "")}
                            </span>
                          );
                        })}
