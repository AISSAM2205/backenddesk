...
import { Bell, RefreshCw, AlertTriangle, Wifi, WifiOff, X, Calendar, Sun, Moon } from 'lucide-react';
...
const TopBar = () => {
  ...
  return (
    <>
      <header style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexShrink: 0 }}>
            <img src="/attijariwafa-logo.png" alt="Attijariwafa" style={{ height: 22, width: 'auto', objectFit: 'contain' }} />

            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ width: 1, height: 24, background: 'var(--b1)' }} />
              <div>
                <div style={{ fontFamily: 'var(--f-disp)', fontWeight: 700, fontSize: '0.72rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--tx1)', lineHeight: 1.2 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 1 }}>
                  {connectionStatus === 'connected' ? (
                    <>
                      ...
                      <span style={{ fontFamily: 'var(--f-disp)', fontSize: '0.57rem', fontWeight: 700, letterSpacing: '0.10em', textTransform: 'uppercase', color: 'var(--profit)' }}>
                        Live
                      </span>
                    </>
                  ) : (
                    <>
                      <span style={{ width: 5, height: 5, borderRadius: '50%', background: 'var(--tx3)' }} />
                      <WifiOff size={10} style={{ color: 'var(--tx3)' }} />
                      <span style={{ fontFamily: 'var(--f-disp)', fontSize: '0.57rem', fontWeight: 700, letterSpacing: '0.10em', textTransform: 'uppercase', color: 'var(--tx3)' }}>
                        Offline
                      </span>
                    </>
                  )}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, flex: 1, justifyContent: 'center' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <span style={{ fontFamily: 'var(--f-mono)', fontSize: '0.80rem', color: 'var(--tx2)', letterSpacing: '0.04em' }}>
                {timeStr}
              </span>
              <span style={{ fontFamily: 'var(--f-body)', fontSize: '0.62rem', color: 'var(--tx3)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                Casablanca
              </span>
            </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            ...
            <button onClick={() => setShowAlerts(v => !v)} style={{
              ...
              background: alerts.length > 0 ? 'rgba(255,43,96,0.10)' : 'transparent',
              border: `1px solid ${alerts.length > 0 ? 'rgba(255,43,96,0.22)' : 'transparent'}`,
              cursor: 'pointer', transition: 'all 0.15s',
              color: alerts.length > 0 ? 'var(--loss)' : 'var(--tx3)',
            }}
              onMouseEnter={e => { if (!alerts.length) { e.currentTarget.style.background = 'var(--surf)'; e.currentTarget.style.color = 'var(--tx2)'; }}}
              onMouseLeave={e => { if (!alerts.length) { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--tx3)'; }}}
            >
              <Bell size={14} />
              {alerts.length > 0 && (
                <span style={{
                  ...
                  fontFamily: 'var(--f-mono)', fontSize: '0.56rem', fontWeight: 700,
            ...
            {/* Theme toggle */}
            <button onClick={toggleTheme} className="btn btn-ghost btn-sm" title={isDark ? 'Mode clair' : 'Mode sombre'}
              style={{ padding: 7 }}>
              {isDark ? <Sun size={13} style={{ color: '#FCD34D' }} /> : <Moon size={13} style={{ color: '#60A5FA' }} />}
            </button>
        ...
        {alerts.length > 0 && (
          <div style={{
            ...
            <AlertTriangle size={12} style={{ color: 'var(--loss)', flexShrink: 0, animation: 'pulse-live 2s ease infinite' }} />
            <span style={{ fontFamily: 'var(--f-disp)', fontSize: '0.60rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--loss)' }}>
              {alerts.length} position{alerts.length > 1 ? 's' : ''} — carry négatif :
            </span>
            <span style={{ fontFamily: 'var(--f-body)', fontSize: '0.72rem', color: '#FC8FA0', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {alerts.slice(0, 6).map(a => a.description || a.isin).join(' · ')}
      ...
      {showAlerts && (
        <div style={{
          position: 'absolute', right: 16, top: alerts.length > 0 ? 76 : 56,
          ...
          <div style={{
            ...
            <span style={{ fontFamily: 'var(--f-disp)', fontWeight: 700, fontSize: '0.68rem', letterSpacing: '0.10em', textTransform: 'uppercase', color: 'var(--tx1)' }}>
              Alertes Carry
            </span>
            <button onClick={() => setShowAlerts(false)} style={{
              background: 'none', border: 'none', cursor: 'pointer', color: 'var(--tx3)', display: 'flex',
            }}>
              <X size={14} />
            </button>
          </div>
          <div style={{ padding: 10, maxHeight: 280, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 6 }}>
            {alerts.length === 0 ? (
              <p style={{ fontFamily: 'var(--f-body)', fontSize: '0.75rem', color: 'var(--tx3)', textAlign: 'center', padding: '16px 0' }}>
                Aucune alerte
              </p>
            ) : alerts.map(a => (
              <div key={a.isin} style={{
                display: 'flex', alignItems: 'flex-start', gap: 10,
                padding: '9px 10px', borderRadius: 8,
                background: 'rgba(255,43,96,0.07)', border: '1px solid rgba(255,43,96,0.18)',
              }}>
                <AlertTriangle size={13} style={{ color: 'var(--loss)', marginTop: 2, flexShrink: 0 }} />
                <div>
                  <div style={{ fontFamily: 'var(--f-body)', fontWeight: 500, fontSize: '0.75rem', color: 'var(--tx1)', lineHeight: 1.3 }}>
                    {a.description || a.isin}
                  </div>
                  <div style={{ fontFamily: 'var(--f-mono)', fontSize: '0.68rem', color: 'var(--loss)', marginTop: 3 }}>
                    Net Daily: {new Intl.NumberFormat('fr-MA', { style: 'currency', currency: 'MAD', maximumFractionDigits: 0 }).format(parseFloat(a.netDailyMad || 0))}
                  </div>
                </div>
              </div>
            ))}
