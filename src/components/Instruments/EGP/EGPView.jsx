        ...
        <div className="card" style={{ overflow: 'hidden' }}>
                ...
                {sorted.map((r, idx) => {
                  ...
                  return (
                    <tr key={r.isin + idx} style={{ background: rowBg }}
                      onMouseEnter={e => e.currentTarget.style.background = 'var(--tr-hover-bg)'}
                      onMouseLeave={e => e.currentTarget.style.background = rowBg}>
                      <td style={{ fontFamily: 'var(--f-mono)', fontSize: '0.68rem', color: '#FCD34D', fontWeight: 500 }}>{r.isin}</td>
                      ...
                      <td style={{ textAlign: 'right', fontFamily: 'var(--f-mono)', fontSize: '0.68rem', color: '#FCD34D' }}>{fN(r.duration, 4)}</td>
                    </tr>
                  );
                })}
              </tbody>
                ...
                <tfoot>
                  <tr>
                    <td colSpan={5} style={{ textAlign: 'left', fontFamily: 'var(--f-disp)', fontWeight: 700, fontSize: '0.60rem', letterSpacing: '0.10em', textTransform: 'uppercase', color: 'var(--tx3)' }}>
                      Total <span style={{ fontFamily: 'var(--f-mono)', color: '#FCD34D' }}>({sorted.length})</span>
                    </td>
          ...
          {egpList.length === 0 && !loading && (
            <div style={{ textAlign: 'center', padding: '48px', color: 'var(--tx3)' }}>
              <Coins size={28} style={{ margin: '0 auto 10px', opacity: 0.3 }} />
              <p style={{ fontFamily: 'var(--f-body)', fontSize: '0.78rem' }}>Aucun snapshot EGP disponible</p>
            </div>
          )}
        </div>
      </div>
