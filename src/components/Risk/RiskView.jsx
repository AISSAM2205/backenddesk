  ...
  const rows = useMemo(() => {
    ...
    const src = dashboardRows
      ...
      .map((r) => ({
        ...
        ...(riskMap[r.isin]
          ? {
              ytmMid: riskMap[r.isin].ytmMid,
              hedgeRatio: riskMap[r.isin].hedgeRatio,
              nbContractsToHedge: riskMap[r.isin].nbContractsToHedge,
              currentFuturesPosition: riskMap[r.isin].currentFuturesPosition,
            }
          : {}),
      }));
    return [...src].sort((a, b) => {
      const na = parseFloat(a[sortKey] || 0),
        nb = parseFloat(b[sortKey] || 0);
      ...
      >
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          ...
          <StatCard
            ...
            value={`${fN(totalDv01, 0)} $/bp`}
            sub="Sensibilité à 1bp"
            valColor="#60A5FA"
          />
          <StatCard
            label="Net Daily ★"
          ...
          <StatCard
            ...
            sub="G-Spread > Target"
            valColor={buySignals.length ? "var(--profit)" : "var(--tx2)"}
          />
        </div>
          ...
          >
            ...
            >
              {RATE_SCENARIOS.map(({ label, delta }) => {
                ...
                const pct =
                  absMax > 0
                    ? Math.min((Math.abs(pnlMadAdj) / absMax) * 100, 100)
                    : 0;
                return (
                  <div
                    key={label}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {[
                ...
                },
                {
                  label: "Net Daily",
                  value: netDaily,
                  col: netDaily >= 0 ? "var(--profit)" : "var(--loss)",
                  sign: true,
                },
              ].map(({ label, value, col }) => {
              ...
              >
                ...
                <div
                  style={{
                    ...
                    fontSize: "0.85rem",
                    color:
                      Math.abs(theta) >= Math.abs(funding)
                        ? "var(--profit)"
                        : "var(--loss)",
                  }}
                >
                  {funding !== 0
                    ? (Math.abs(theta / funding) * 100).toFixed(0)
                    : "—"}
                  %
                </div>
                <div
                ...
                {displayRows.map((r, idx) => {
                  ...
                  const rowBg = isAlert
                    ...
                    : idx % 2 === 0
                      ...
                      : "transparent";
                  const spread =
                    parseFloat(r.targetSpread || 0) > 0
                      ? (
                          parseFloat(r.gSpreadBid || 0) -
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ textAlign: "center" }}>
                        {r.subAsset && (
                          <span
                            className={`badge ${(r.subAsset || "").toLowerCase().includes("ocp") ? "badge-fut" : "badge-eb"}`}
                          >
                            {r.subAsset}
                          </span>
                        )}
