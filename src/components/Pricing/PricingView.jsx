...
import { RefreshCw, TrendingUp, TrendingDown, Minus, AlertTriangle, Target } from 'lucide-react';

/* ─── Formatters ─────────────────────────────────────────── */
const fN  = (v, d = 1) => v == null || isNaN(parseFloat(v)) ? '—' : parseFloat(v).toFixed(d);
const fBp = (v, d = 1) => v == null || isNaN(parseFloat(v)) ? '—' : `${parseFloat(v) >= 0 ? '+' : ''}${parseFloat(v).toFixed(d)}`;
const fPx = (v)        => v == null || isNaN(parseFloat(v)) ? '—' : parseFloat(v).toFixed(3);
const fPct = (v, d = 2) => v == null || isNaN(parseFloat(v)) ? '—' : `${parseFloat(v).toFixed(d)}%`;
...
const SignalChip = ({ signal }) => {
  ...
  }[signal] || { bg: 'rgba(100,116,139,0.08)', bd: 'rgba(100,116,139,0.18)', col: 'var(--tx3)', lbl: '—', icon: Minus };
  const Icon = cfg.icon;
  return (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4,
      ...
      <span style={{ fontFamily: 'var(--f-disp)', fontSize: '0.55rem', fontWeight: 800,
        letterSpacing: '0.08em', color: cfg.col }}>{cfg.lbl}</span>
    </div>
  );
};
...
const SpreadBar = ({ bid, ask, mid, target, color }) => {
  if (!bid && !ask) return <span style={{ color: 'var(--tx3)', fontSize: '0.62rem' }}>—</span>;
  const bv = parseFloat(bid || 0), av = parseFloat(ask || 0), mv = parseFloat(mid || ((bv+av)/2));
  const tv = parseFloat(target || mv);
  const range = Math.max(av - bv, 1);
  const midPct   = ((mv - bv) / range * 100).toFixed(0);
  const tgtPct   = Math.max(0, Math.min(100, ((tv - bv) / range * 100))).toFixed(0);
  return (
    <div style={{ minWidth: 120 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
        <span style={{ fontFamily: 'var(--f-mono)', fontSize: '0.58rem', color: 'var(--loss)' }}>{fBp(bid)}</span>
        <span style={{ fontFamily: 'var(--f-mono)', fontSize: '0.62rem', fontWeight: 700, color }}>{fBp(mid || mv)}</span>
        <span style={{ fontFamily: 'var(--f-mono)', fontSize: '0.58rem', color: 'var(--profit)' }}>{fBp(ask)}</span>
      </div>
      <div style={{ position: 'relative', height: 4, borderRadius: 2, background: 'var(--elev)', overflow: 'visible' }}>
        ...
        {/* Mid tick */}
        <div style={{ position: 'absolute', top: -2, left: `${midPct}%`,
          transform: 'translateX(-50%)', width: 2, height: 8, borderRadius: 1,
          background: color }} />
        {/* Target tick */}
        {target != null && (
          <div style={{ position: 'absolute', top: -3, left: `${tgtPct}%`,
      ...
      {target != null && (
        <div style={{ textAlign: 'center', marginTop: 2 }}>
          <span style={{ fontFamily: 'var(--f-mono)', fontSize: '0.54rem', color: 'var(--warn)' }}>
            ▲ cible {fBp(target)} bp
          </span>
        </div>
      )}
...
const PricingView = () => {
  ...
  const pricingRows = useMemo(() => {
    ...
    return (dashboardRows || [])
      .filter(r => (r.subAsset || '').toLowerCase().includes('bond') || (r.subAsset || '').includes('Bond'))
      .filter(r => !filter || r.isin?.includes(filter.toUpperCase()) || (r.description||'').toLowerCase().includes(filter.toLowerCase()))
      .map(r => {
        ...
        const signal = cheapness > 15 ? 'CHEAP' : cheapness < -15 ? 'RICH' : cheapness > 5 ? 'BUY' : cheapness < -5 ? 'SELL' : 'HOLD';
        ...
        return {
          ...
          rollTime:  r.maturityDate ? ((new Date(r.maturityDate) - new Date()) / (1000*60*60*24*365)).toFixed(2) : null,
        };
      })
  ...
  const TH  = { padding: '7px 10px', fontFamily: 'var(--f-disp)', fontWeight: 700, fontSize: '0.54rem',
    ...
    borderBottom: '1px solid var(--b1)', whiteSpace: 'nowrap', textAlign: 'right', background: 'var(--surf)' };
  const THL = { ...TH, textAlign: 'left' };
  const TD  = (col) => ({ padding: '8px 10px', fontFamily: 'var(--f-mono)', fontSize: '0.68rem',
    borderBottom: '1px solid var(--b0)', whiteSpace: 'nowrap', color: col || 'var(--tx1)',
  ...
  return (
    <div style={{ padding: '16px 20px', height: '100%', overflowY: 'auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
        <div>
          <h1 style={{ fontFamily: 'var(--f-disp)', fontWeight: 800, fontSize: '1.0rem',
            color: 'var(--tx1)', letterSpacing: '-0.01em', margin: 0 }}>
            Pricing Screen
          </h1>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <input
            ...
            style={{
              background: 'var(--surf)', border: '1px solid var(--b1)', borderRadius: 5,
              padding: '6px 10px', fontFamily: 'var(--f-body)', fontSize: '0.72rem',
              color: 'var(--tx1)', outline: 'none', width: 200,
            }}
          />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 14 }}>
        {[
          { label: 'G-Spread Moy.', value: fBp(pricingRows.reduce((s,r)=>s+r.gMid,0)/Math.max(pricingRows.length,1)) + ' bp', color: 'var(--eb)' },
          { label: 'I-Spread Moy.', value: fBp(pricingRows.reduce((s,r)=>s+r.iMid,0)/Math.max(pricingRows.length,1)) + ' bp', color: 'var(--cyan)' },
          { label: 'Lignes CHEAP',  value: pricingRows.filter(r=>r.signal==='CHEAP'||r.signal==='BUY').length,  color: 'var(--profit)' },
          { label: 'Lignes RICH',   value: pricingRows.filter(r=>r.signal==='RICH'||r.signal==='SELL').length,  color: 'var(--loss)'   },
        ].map(k => (
          <div key={k.label} className="card" style={{ padding: '10px 13px' }}>
            <div style={{ fontFamily: 'var(--f-disp)', fontSize: '0.51rem', fontWeight: 700,
              letterSpacing: '0.10em', textTransform: 'uppercase', color: 'var(--tx3)', marginBottom: 5 }}>{k.label}</div>
            <div style={{ fontFamily: 'var(--f-mono)', fontWeight: 700, fontSize: '1.0rem',
        ...
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            style={{
              ...
              borderBottom: tab === t.id ? '2px solid var(--eb)' : '2px solid transparent',
              ...
              {pricingRows.map((r, i) => {
                ...
                return (
                  <tr key={r.isin}
                    ...
                    <td style={TD('var(--tx2)')}>{r.maturityDate?.substring(0,7) || '—'}</td>
                    <td style={TD('var(--tx3)')}>{r.rollTime ? `${r.rollTime}Y` : '—'}</td>
                    <td style={{ ...TD(), paddingTop: 6, paddingBottom: 6 }}>
                      <SpreadBar bid={r.gBid} ask={r.gAsk} mid={r.gMid}
                        target={r.targetSpreadCalc} color="var(--eb)" />
                    </td>
                    ...
                    <td style={{ padding: '6px 10px', textAlign: 'right' }}>
                      <input
                        type="number"
                        value={editTargets[r.isin] ?? Math.round(r.targetSpreadCalc)}
                        onChange={e => updateTarget(r.isin, e.target.value)}
                        style={{
                          width: 60, textAlign: 'right', background: 'rgba(255,255,255,0.04)',
                          border: '1px solid var(--b1)', borderRadius: 3,
                          padding: '3px 6px', color: 'var(--warn)',
                          fontFamily: 'var(--f-mono)', fontSize: '0.65rem', outline: 'none',
                        }}
                      />
                    </td>
                    <td style={{ ...TD(), background: rc.bg, color: rc.col, fontWeight: 700 }}>
                      {r.cheapness >= 0 ? '+' : ''}{fBp(r.cheapness)} bp
                    </td>
                    <td style={{ padding: '6px 10px', textAlign: 'right' }}>
              ...
              {pricingRows.map((r, i) => {
                ...
                return (
                  <tr key={r.isin}
                    ...
                    <td style={TD(spreadDiff > 0 ? 'var(--profit)' : 'var(--loss)')}>
                      {fBp(spreadDiff)} bp
                    </td>
            ...
            <tbody>
              {pricingRows.map((r, i) => (
                <tr key={r.isin}
                  ...
                  <td style={TD(parseFloat(r.perfWap||0)>=0?'var(--profit)':'var(--loss)')}>
                    {fBp(r.perfWap)} bp
                  </td>
      ...
      {tab === 'relative' && pricingRows.length > 0 && (
        <div>
          <div style={{ marginBottom: 14 }}>
            <div style={{ fontFamily: 'var(--f-body)', fontSize: '0.68rem', color: 'var(--tx3)', lineHeight: 1.5 }}>
              Écart G-Spread vs cible trader. <span style={{ color: 'var(--profit)' }}>Vert = cheap</span> (G-Spread &gt; cible — opportunité achat).{' '}
              <span style={{ color: 'var(--loss)' }}>Rouge = rich</span> (G-Spread &lt; cible — opportunité vente).
            </div>
          </div>
          ...
          <div style={{ overflowX: 'auto', marginBottom: 20 }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {[...pricingRows].sort((a, b) => b.cheapness - a.cheapness).map((r, i) => {
                  const rc = relColor(r.cheapness);
                  const barW = Math.min(Math.abs(r.cheapness) / 40 * 100, 100);
                  return (
                    <tr key={r.isin}>
                      <td style={{ ...TDL('var(--tx3)'), fontWeight: 700 }}>{i + 1}</td>
                      <td style={TDL('var(--tx2)')}>{r.description || r.isin}</td>
                      <td style={TD()}>{fBp(r.gMid)} bp</td>
                      <td style={TD('var(--warn)')}>{fBp(r.targetSpreadCalc)} bp</td>
                      ...
                      <td style={{ padding: '8px 10px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          {r.cheapness >= 0 ? (
                            <div style={{ width: `${barW}%`, maxWidth: 180, height: 6, borderRadius: 3,
                              background: 'var(--profit)', opacity: 0.7, marginLeft: 'auto',
                              direction: 'rtl' }} />
                          ) : (
                            <div style={{ width: `${barW}%`, maxWidth: 180, height: 6, borderRadius: 3,
                              background: 'var(--loss)', opacity: 0.7 }} />
                          )}
                        </div>
                      </td>
                      <td style={TD()}>{fN(r.modifiedDuration, 2)} Y</td>
