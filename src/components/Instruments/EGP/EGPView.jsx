...
const NdfCurvePanel = ({ rates }) => {
  ...
  const usdRate = parseFloat(rates?.sofr || 5.3) / 100; // SOFR
  ...
  return (
    <div className="card slide-up stagger-3" style={{ padding: "14px 16px" }}>
      ...
      >
        ...
        >
          Spot {spot.toFixed(2)} · CBE {(egpRate * 100).toFixed(2)}% · SOFR{" "}
          {(usdRate * 100).toFixed(2)}%
        </span>
      </div>
      ...
      >
        Fwd = Spot × (1 + r_EGP × t/360) / (1 + r_USD × t/360). Taux CBE/SOFR en
        vigueur.
      </p>
    </div>
  );
